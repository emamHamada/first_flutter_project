...
import '../../../controllers/note_controllers/add_task_controller.dart';
import '../../../controllers/note_controllers/task_controller.dart';
import '../../../models/note_models/task.dart';
import '../../components_to_use/constants/constants.dart';
import '../../components_to_use/reused/note_reused/note_button.dart';
import '../../components_to_use/reused/note_reused/note_input_field.dart';

class AddTaskPage extends GetWidget {
              ...
              builder: (logic) {
                return Column(
                  children: [
                    ...
                    InputField(
                      label: ' Date',
                      note: DateFormat.yMd().format(logic.selectedDateTime),
                      widget: IconButton(
                        onPressed: () => logic.getDateFromUser(context),
                        icon: const Icon(
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: InputField(
                            label: 'StartTime',
                            note: logic.startTime,
                            widget: IconButton(
                              //_getTimeFromUser(isStartTime: true),
                              onPressed: () => logic.getTimeFromUser(
                                isStartTime: true,
                                context: context,
                              ),
                              icon: const Icon(
                    ...
                    InputField(
                      label: ' Remind',
                      note:
                          '${logic.selectedRemind.toString()}   minutes early',
                      widget: logic.buildDrawableItemsList(),
                    ),
                    InputField(
                      label: 'Repeat',
                      note: '${logic.selectedRepeat.toString()} ',
                      widget: logic.buildDrawableItemsList0(),
                    ),
                    const SizedBox(
                      height: 18,
                    ),
                    Row(
                      ...
                      children: [
                        logic.colorPalette(context),
                        MyButton(
                          onTap: () {

                            _validateData(
                              selectedDateTime: logic.selectedDateTime,
                              startTime: logic.startTime,
                              endTime: logic.endTime,
                              selectedColor: logic.selectedColor,
                              selectedRemind: logic.selectedRemind,
                              selectedRepeat: logic.selectedRepeat,
                            );
  ...
  _addTasksToDatabase({
    ...
    int? value = await _taskController.addTask(
      task: Task(
        ...
        isCompleted: 0,
        date: DateFormat.yMd().format(selectedDateTime),
        startTime: startTime,
        endTime: endTime,
        color: selectedColor,
...
class _ImagesState extends State<Images> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: GetBuilder<CustomDrawerController>(
        ...
        builder: (controller) {
          return Column(
            ...
            children: [
              controller.pickedImage == null ? const FlutterLogo() : Image.file(
...
class _AddTaskPageState extends State<AddTaskPage> {
  ...
  AppBar _buildAppBar() => AppBar(
    ...
    actions: const [
      CircleAvatar(
        backgroundImage: AssetImage("assets/icons/aaa.png"),
        radius: 24,
      ),
      SizedBox(
        width: 10,
